 ...
 */
public class Datagram implements Iterable<String>{
     ...
     */
    public Datagram(LinkedHashMap<String, ArrayList<String>> info, String nom) {
        name=nom;
        this.info= info;
        longLabel=0;
        longInfo=0;
   ...
   public String getName()
    ...
    /**
     * Add a new key(label) in the Hashmap
    ...
    /**
     * Increase the number of rows if is the first position and add the info to a specific key
     * @param pos row to add the info
    ...
    /**
     * Access to the info in the dataframe of a single position (row) in the arrayList of a key label (name)
     * @param row number with the row selected
     * @param col key label name
     * @return value of a single item (row) and key label (name)
     */
    ...
    /**
     * Get the content of the dataframe
     * @return info (the content of the dataframe)
     */
